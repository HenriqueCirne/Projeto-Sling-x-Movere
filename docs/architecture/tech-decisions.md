...
| R5 | ~~Limites das faixas de Prazo Médio (FR5) — não documentados em lugar nenhum~~ **RESOLVIDO 2026-09-03** — gestor forneceu os limites (P4 0-34 dias, P5 35-47, P6 48-62, P7 63-77, P8 78-92, P9 93-109, P1 acima de 109 dias). Story 2.4 implementada | Não bloqueia mais nada | — |
...
| 2026-09-02 | 1.5 | Story 1.4 implementada (feature `erp-sync` + CLI `sync:moveres`) — ver Dev Agent Record da story para detalhe completo. `replaceSalesEntriesWindow` (TD-04b) extraída de `sales-import` para `shared/sales-entries/`, agora com escopo de loja de verdade (a importação de planilha só escopava por data — gap real corrigido nesta extração). Smoke test real (`2026-08-03`, 10 lojas, 0 erros) revelou um trade-off pendente de decisão do usuário: linhas do ERP não têm família/grupo/marca/linha nem condição de pagamento (pendências do Achado 8) — um backfill histórico amplo via ERP degradaria essas dimensões nos relatórios para o período resincronizado, diferente da planilha (TD-07) que já traz tudo completo. Nenhum backfill amplo foi rodado; só o smoke test de um dia | Dex (@dev) |
| 2026-09-03 | 1.6 | R5 resolvido: gestor forneceu os limites das faixas de Prazo Médio (FR5). Story 2.4 implementada — ver `docs/stories/2.4.story.md`. Achado técnico: `prazoMedio` fracionário (Decimal(8,2), desde a Story 1.4) exige faixas em cascata (teto único por faixa), não intervalos `[min,max]` fixos — um par fixo deixa buraco para valores entre tetos consecutivos (ex: 34,5 entre 34 e 35) | Dex (@dev) |
